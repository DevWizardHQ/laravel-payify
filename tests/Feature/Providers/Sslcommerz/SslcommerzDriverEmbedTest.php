...
it('does not leak store credentials in embedAttributes', function () {
    $driver = sslcommerzDriverWith(new MockHandler([]));

    $attrs = $driver->embedAttributes(new PaymentRequest(
        amount: 100, currency: 'BDT', reference: 'EMB-SEC',
        customer: new Customer(name: 'X', email: 'x@y.z', phone: '0', city: 'Dhaka', country: 'BD'),
        callbackUrl: 'https://cb',
    ));

    $postdata = json_decode($attrs['data-postdata'], true);
    expect($postdata)->not->toHaveKey('store_passwd');
    expect($postdata)->not->toHaveKey('store_id');
    expect($attrs['data-postdata'])->not->toContain('qwerty');
    expect($attrs['data-postdata'])->not->toContain('testbox');
});
